...
savedir = r'path/CMCNet_DeepCrack'  # 基础保存路径

imgpath = r'DeepCrack1/train_img'
labpath = r'DeepCrack1/train_lab'
imgsz = 512
...
lossx = 0
tp, tn, fp, fn = 0, 0, 0, 0
accuracy, precision, recall, F1, ls_loss, ious = [], [], [], [], [], []
...
def train():
    for epoch in range(items):
        ...
        for idx, samples in enumerate(trainsets):
            ...
            iou_batch = compute_iou(tp_, fp_, fn_)
            epoch_ious.append(iou_batch)

        accuracy_, precision_, recall_, F1_ = compute_indexes(tp, fp, tn, fn)
        iou_ = compute_iou(tp, fp, fn)
        ...
        print(f"Epoch {epoch + 1}/{items}, Loss: {lossx:.4f}, "
              ...
              f"Recall: {recall_:.4f}, F1: {F1_:.4f}, IoU: {iou_:.4f}")
