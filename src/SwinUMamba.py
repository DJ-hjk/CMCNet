...
class PatchEmbed2D(nn.Module):
    ...
    """
    def __init__(self, patch_size=8, in_chans=3, embed_dim=64, norm_layer=None, **kwargs):
        super().__init__()
...
class VSSMEncoder(nn.Module):
    def __init__(self, patch_size=8, in_chans=3, depths=[2, 2, 9, 2], 
                 dims=[64, 128, 256, 512], d_state=16, drop_rate=0., attn_drop_rate=0., drop_path_rate=0.2,
        ...
        for i_layer in range(self.num_layers):
            layer = VSSLayer(
                ...
                d_state=math.ceil(dims[0] / 6) if d_state is None else d_state, # 20240109   这行代码的主要目的是为 d_state 提供一个默认值。如果在调用时没有提供 d_state（即它为 None），则根据 dims[0] 计算出一个新的值；如果提供了 d_state，则直接使用传入的值。
                drop=drop_rate, 
                attn_drop=attn_drop_rate,
                drop_path=dpr[sum(depths[:i_layer]):sum(depths[:i_layer + 1])],#这行代码的主要作用是根据当前层的深度信息，从 dpr 中提取出与当前层相关的值，通常用于深度学习模型中的某些操作，如残差连接或路径选择。
                norm_layer=norm_layer,
                downsample=None,
                use_checkpoint=use_checkpoint,
            )
            self.layers.append(layer)
            if i_layer < self.num_layers:#这个条件用于判断当前层是否是倒数第二层。如果是，则执行下面的语句。  若是倒数第二层之前的都添加一个下采样模块
    ...
    def forward(self, x):
        ...
        for s, layer in enumerate(self.layers):
            ...
            if s < len(self.downsamples):#如果当前层的索引 s 小于下采样层的数量，则将当前特征图 x 传递给对应的下采样层 self.downsamples[s]，以进行下采样处理。
                x = self.downsamples[s](x)

        import torch.nn as nn

        downsampler = nn.AvgPool2d(kernel_size=2, stride=2)
        x = downsampler(x)
...
class SwinUMamba(nn.Module):
    def __init__(
        ...
        drop_path_rate=0,
        layer_scale_init_value=1e-6,#层缩放的初始值。用于在训练初期对某些层的输出进行缩放，帮助稳定训练过程。
        hidden_size: int = 512,
        norm_name = "instance",
        res_block: bool = True,
        spatial_dims=2,
        deep_supervision: bool = False,#是否使用深度监督。如果为 True，则在网络的不同层输出中间特征图，以便在训练时提供额外的监督信号，从而提高模型的性能。
    ) -> None:
        super().__init__()
    ...
    def forward(self, x_in):
        ...
        """
        return vss_outs[0],vss_outs[1],vss_outs[2],vss_outs[3],vss_outs[4]
